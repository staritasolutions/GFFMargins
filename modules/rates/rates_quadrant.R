    ...
    moduleServer(id, function(input, output, session){
      ...
      labels_df <- reactive({
        ...
        tibble(
          ...
          x_point = c(x_range[2] - x_pad, x_range[1], x_range[1], x_range[2] - x_pad),
          y_point = c(y_range[2] - y_pad, y_range[2] - y_pad, y_range[1] - y_pad, y_range[1] + y_pad)
        )
      })
      ...
      ggplot(data = plt_df(), aes(x = Hours, y = Revenue, color = quadrant)) +
        ...
        geom_label(data = labels_df(), aes(x = x_point, y = y_point, label = label, color = label),
           inherit.aes = FALSE, size = 2, fill = "white", alpha = 0.8) + 
        labs(x = "Hours",
             y = "Revenue") +
